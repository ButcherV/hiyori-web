...
export function QuickActions({
  ...
  return (
    <div className={styles.container}>
      {/* Mode row: format toggle + now + special hours */}
      <div className={styles.row}>
        ...
        {specialHours.map((item, i) => (
          <button
            key={`h-${i}`}
            className={`${styles.chip} ${styles.chipHour}`}
      ...
      {/* Minute row: special minutes */}
      <div className={styles.row}>
        {specialMinutes.map((item, i) => (
          <button
            key={`m-${i}`}
            className={`${styles.chip} ${styles.chipMinute}`}
            onClick={() => onJumpTo(item.hour, item.minute)}
          >
            {item.label}
          </button>
