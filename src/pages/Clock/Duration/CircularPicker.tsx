...
export function CircularPicker({ startAngle, endAngle, selectedPeriod, onGoToCurrent }: CircularPickerProps) {
  const getArcPath = () => {
  ...
  const rangeText = (() => {
    if (selectedPeriod) {
      if (selectedPeriod.start === selectedPeriod.end) return `${selectedPeriod.start}:00`;
        ...
        <g>
          {labels.map((label: any, i) => (
            <text key={i} x={label.x} y={label.y} textAnchor="middle"
              dominantBaseline="central" fill="#888888" fontSize="14" fontWeight="500">
              {label.text}
            </text>
          ))}
        </g>
  ...
  return (
    <div className={styles.clockContainer}>
      <svg className={styles.clockSvg} viewBox="0 0 400 400">
        ...
        <path d={getArcPath()} fill="none" stroke="#a9e55b" strokeWidth="50" strokeLinecap="butt" />
        {staticOverlay}
      </svg>
      ...
      <div className={styles.centerOverlay}>
        ...
        <div className={styles.kanjiBlock}>
          {selectedPeriod && (
            <span className={`${styles.kana} jaFont`}>{selectedPeriod.kana}</span>
          )}
